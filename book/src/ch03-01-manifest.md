...
```python
"step_budget": {
    "max_steps": 20,                  # étapes (appels outils + appels LLM) — défaut runtime : 10
    "max_tool_calls": 40,             # appels d'outils uniquement — défaut runtime : 20
    "wall_clock_timeout_secs": 300,   # temps réel en secondes — défaut runtime : 300
},
```
