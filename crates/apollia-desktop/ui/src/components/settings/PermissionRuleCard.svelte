...
<EntityCard
  iconTone={isAllow ? "success" : "destructive"}
...
>
  {#snippet icon()}
    {#if isAllow}
      <ShieldCheck size={16} aria-hidden="true" class="text-success" />
    {:else}
      <ShieldX size={16} aria-hidden="true" class="text-destructive" />
    {/if}
  {/snippet}
  ...
  {#snippet body()}
    <dl class="space-y-0.5 text-[12px] text-muted-foreground">
      {#if rule.arg_prefix}
        <div>
