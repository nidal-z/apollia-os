...
impl AgentRole {
    fn parse(s: &str) -> Result<Self, PackageLoaderError> {
        match s {
            ...
            // `standalone` is the term forge / SDK templates use for an agent
            // that exposes skills but isn't part of a Director hierarchy.
            // Semantically that's the worker role (single-agent package),
            // so we accept it as an alias rather than rejecting valid
            // manifests generated by `/apollia-worker-forge`.
            "worker" | "standalone" => Ok(AgentRole::Worker),
            "assistant" => Ok(AgentRole::Assistant),
            other => Err(PackageLoaderError::InvalidManifest(format!(
                "rôle inconnu '{other}' — attendu : director | worker | standalone | assistant"
            ))),
        }
    }
}
