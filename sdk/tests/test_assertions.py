...
def _completed_result(text: str = "ok") -> dict:
    return {"status": "completed", "parts": [{"type": "text", "content": text}]}
...
class TestAssertResultCompleted:
    ...
    def test_fails_with_message_on_wrong_status(self) -> None:
        ...
        with pytest.raises(AssertionError, match="Expected result status 'completed', got 'failed'"):
            assert_result_completed(result)
    ...
    def test_contains_fails_when_substring_missing(self) -> None:
        ...
        with pytest.raises(AssertionError, match="Expected result text to contain 'Goodbye'"):
...
class TestAssertResultFailed:
    ...
    def test_fails_on_wrong_error_code(self) -> None:
        result = _failed_result("TIMEOUT")
        with pytest.raises(AssertionError, match="Expected error code 'TOOL_ERROR', got 'TIMEOUT'"):
            assert_result_failed(result, code="TOOL_ERROR")

    def test_fails_on_completed_status(self) -> None:
        result = _completed_result()
        with pytest.raises(AssertionError, match="Expected result status 'failed', got 'completed'"):
            assert_result_failed(result)
...
class TestAssertResultInputRequired:
    ...
    def test_fails_on_completed(self) -> None:
        result = _completed_result()
        with pytest.raises(AssertionError, match="Expected result status 'input_required', got 'completed'"):
            assert_result_input_required(result)


class TestAssertToolCalled:
    """Tests for assert_tool_called."""
    ...
    def test_fails_when_tool_never_called(self) -> None:
        ...
        with pytest.raises(AssertionError, match="Expected tool 'bash' to be called, but it was never called"):
            assert_tool_called(ctx, "bash")

    def test_fails_when_tools_is_none(self) -> None:
        ctx = MockContext.create()

        with pytest.raises(AssertionError, match="MockContext has no tools configured"):
            assert_tool_called(ctx, "bash")

    def test_counts_only_matching_tool(self) -> None:
        ctx = MockContext.create(tools={"bash": {"output": ""}, "file_io": {"data": ""}})
...
class TestAssertLlmCalled:
    ...
    def test_passes_when_llm_called(self) -> None:
        ctx = MockContext.create(llm_responses=[{"content": "hi"}])
    ...
    def test_fails_when_never_called(self) -> None:
        ...
        with pytest.raises(AssertionError, match="Expected LLM to be called, but it was never called"):
            assert_llm_called(ctx)
    ...
    def test_fails_with_wrong_count(self) -> None:
        ...
        with pytest.raises(AssertionError, match="Expected LLM to be called 5 times, was called 2 times"):
            assert_llm_called(ctx, times=5)

    def test_fails_when_llm_is_none(self) -> None:
        ctx = MockContext.create()

        with pytest.raises(AssertionError, match="MockContext has no LLM configured"):
            assert_llm_called(ctx)
