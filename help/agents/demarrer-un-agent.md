...
1. Dans la sidebar, cliquez sur **Agents**, puis sur l'onglet **Mes agents**.
   `[SCREENSHOT: onglet Mes agents avec sections Assistants et Workers, cartes agents avec statut gris "Prêt"]`

2. Localisez votre agent dans la bonne section :
   - **Assistants** pour les agents conversationnels.
   - **Workers** pour les agents appelés par les triggers ou les pipelines.

3. Cliquez sur **Démarrer** sur la carte de l'agent. Le statut passe au vert et le bouton se transforme en **Arrêter**.
   `[SCREENSHOT: carte agent avec statut vert "Démarré" et bouton "Arrêter" + bouton secondaire "Ouvrir le chat"]`
...
Le statut de la carte agent est vert et indique **Démarré**. Pour un Assistant, l'envoi d'un message dans la conversation déclenche une réponse en streaming.
