...
[ ] Agent absent après arrêt
    ⌨  apollia-os agent list
    →  Liste vide (ou agent absent)

[ ] Redéploiement après arrêt
    ⌨  apollia-os agent start agents/apollia-reviewer.py
...
```
[ ] Tâche synchrone complète
    ⌨  apollia-os run apollia-reviewer "$(pwd)"
    →  Exit code 0
    →  Affiche un rapport Markdown commençant par "# Review —"
...
[ ] Exit code succès
    ⌨  apollia-os run apollia-reviewer "$(pwd)"; echo "Exit: $?"
    →  Exit: 0
