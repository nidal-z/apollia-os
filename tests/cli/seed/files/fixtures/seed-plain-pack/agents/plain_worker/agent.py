"""seed-plain-pack / seed-plain-worker, packaged worker stub for the install dialog fixtures.

Never started by the seed: the package sits under fixtures/ so the dialog can
preview it and install it from a picker the automaton answers. Same VALID shape as the
...
@agent(
    name="seed-plain-worker",
    version="0.1.0",
    description="Seed fixture worker with no dependency.",
    tags=("worker", "seed", "fixture"),
    memory_namespace="seed-plain-worker",
    agent_type="worker",
)
class SeedPlainWorker:
    @skill(
        "classify_text",
