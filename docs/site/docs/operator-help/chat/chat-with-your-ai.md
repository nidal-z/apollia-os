...
9. If the AI wants to perform a sensitive action (write a file, run a command), an approval card appears: see [Approve or reject an action](../controle/approve-or-reject-an-action.md).
...
- **Answer in error or truncated:** switch models in the configuration panel, or read [The AI provider does not respond](../troubleshooting/the-ai-provider-does-not-answer.md).
