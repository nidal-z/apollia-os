...
Click an approval row to expand it into a decision card. See the dedicated page [Approve or refuse an agent action](../controle/approve-or-reject-an-action.md) for the detail of the scopes of *Always allow* and of the refusal reason dialog (5 to 500 characters).
...
A selector at the top filters by channel. To configure channels or global events, go to **Notifications** (a top-level entry in the sidebar) (see [Configure a channel](../notifications/set-up-a-channel.md)).
...
- **You want to avoid re-approving the same action over and over**: use the *Always allow* options on the card. See [Approve or refuse an agent action](../controle/approve-or-reject-an-action.md).
