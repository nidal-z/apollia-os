...
| **apollia-llm** | The multi-backend LLM router: local plus remote (Anthropic, OpenAI, Mistral, and any OpenAI-compatible endpoint, Ollama included), daily cost tracking, a Hugging Face GGUF registry, hardware detection, and the model recommender that ranks GGUF files for the local machine from their headers, their memory cost and a curated generation table. |
