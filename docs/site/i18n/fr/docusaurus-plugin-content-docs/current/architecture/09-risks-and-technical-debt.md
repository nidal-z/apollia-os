...
| **Recommandation de modèles** | L'onboarding classe les modèles pour la machine locale, mais trois de ses entrées relèvent du jugement plutôt que de la mesure : les scores de capacité de la table de générations (`families.toml`), la part de capacité que conserve chaque quantization, et les débits mémoire derrière le terme de vitesse. Les coûts mémoire, la compatibilité et le placement des couches sont calculés à partir des fichiers eux-mêmes. Le catalogue est récupéré en direct, si bien qu'une machine sans réseau ne se voit proposer aucune liste. Les fichiers GGUF fragmentés ne sont pas proposés, car le téléchargeur ne récupère qu'une URL. |
