...
1. Dans la sidebar, ouvrez **Mes assistants**. La page liste vos assistants existants, et le bouton **Nouvel assistant** est en haut à droite.
   ...
   ![Page Mes assistants : liste à gauche, détail de l'agent sélectionné à droite, bouton "Nouvel assistant" en haut à droite](/img/operator-help/fr/agents-installer-un-agent-1.png)
...
4. **Aperçu du package.** Apollia affiche un résumé : nom, version, auteur, la liste des agents du package, leurs déclencheurs (s'il y en a) et le nombre de dépendances. Prenez le temps de vérifier que ça correspond bien à ce que vous attendez.
   ...
   ![Dialogue d'installation, étape preview : sections Agents et Triggers, badge vert Valide](/img/operator-help/fr/agents-installer-un-agent-2.png)
   ...
   ![Aperçu avec un trigger webhook nécessitant une configuration, bouton Configurer →](/img/operator-help/fr/agents-installer-un-agent-2bis.png)
...
6. **(Optionnel) Configuration des webhooks.** Si on vous le demande, chaque webhook nécessite un **secret** (au moins 32 caractères) qui sécurise les appels entrants. Trois cas :
   ...
   ![Dialogue d'installation, étape configure : carte d'un trigger webhook avec endpoint URL et champ secret HMAC-SHA256](/img/operator-help/fr/agents-installer-un-agent-3.png)
...
7. Cliquez sur **Installer**. Apollia copie le package, enregistre les agents et active leurs déclencheurs. Un écran final confirme l'installation avec le nombre d'agents et de déclencheurs créés.
   ...
   ![Écran de confirmation Package installé ! avec compteur agents et triggers, bouton Fermer](/img/operator-help/fr/agents-installer-un-agent-4.png)
