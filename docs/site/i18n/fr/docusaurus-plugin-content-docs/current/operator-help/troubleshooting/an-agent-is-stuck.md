...
2. Onglet **À traiter** par défaut. Filtrez sur la chip **Approbations** pour ne voir que les approbations en attente, et utilisez le sélecteur **Agent** à droite pour isoler l'agent concerné.
   ![Boite de reception sur l'onglet A traiter, une carte d'approbation depliee montrant ce que l'agent attend](/img/operator-help/troubleshooting-un-agent-est-bloque-1.png)
3. Cliquez sur l'item pour le déplier en carte HITL, puis **Autoriser** / **Refuser**. L'agent reprend immédiatement son travail. Voir [Approuver ou refuser une action d'agent](../controle/approve-or-reject-an-action.md) pour le détail.
...
> **Référence technique :** [Référence Apollia](/reference) - comprendre le superviseur qui surveille l'avancement des agents et déclenche les timeouts.
