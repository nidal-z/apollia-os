...
- **La carte Microsoft 365 affiche « Configuration requise »** : elle ne devrait pas, puisque l'identifiant est fourni avec Apollia. Vérifiez si `APOLLIA_MICROSOFT_CLIENT_ID` est exportée à vide dans le shell qui a lancé Apollia, ou si `~/.apollia/oauth-clients.toml` contient une entrée `[microsoft]` vidée à la main. Les deux priment sur l'identifiant fourni.
