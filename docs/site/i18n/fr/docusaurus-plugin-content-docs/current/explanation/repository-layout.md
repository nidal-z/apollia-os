...
| `tests/` | La crate de tests d'intégration et de bout en bout au niveau de l'espace de travail (`apollia-e2e-tests`), ainsi que les suites CLI (`tests/cli/`) et Python (`tests/python/`) qui exercent le runtime à travers les frontières des crates. Les tests unitaires par crate vivent quant à eux à l'intérieur de chaque crate. |
...
| `.pre-commit-config.yaml` | Hooks pre-commit : `ruff format`, `ruff check`, `rustfmt`, `clippy`, et `cargo check`. À ne jamais contourner. |
