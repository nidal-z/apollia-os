...
3. Cliquez sur l'icône **⋯** (trois points) à droite de la ligne → **Voir l'historique**. Un panneau coulissant s'ouvre depuis la droite, intitulé **Historique des déclenchements**, avec un compteur en haut indiquant le nombre total d'événements.
   `[SCREENSHOT: ligne d'automatisation au hover, menu trois points ouvert avec "Voir l'historique" surligné, panneau d'historique en train de s'ouvrir à droite]`
...
4. Chaque ligne de la liste contient déjà l'essentiel — pas besoin de cliquer pour ouvrir un détail :
   ...
   `[SCREENSHOT: panneau Historique des déclenchements ouvert — chips de filtre statut en haut, liste de cartes empilées : un DÉCLENCHÉ vert, un IGNORÉ orange, un ERREUR rouge avec sa raison en dessous]`
