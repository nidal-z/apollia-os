...
--
-- Timestamps are relative, `datetime('now', '-N minutes')`, and that is the
-- point rather than a detail. The rest of the seed hardcodes 2026-07-01, which
-- was already thirty days old when this was written and drifts one day further
-- every day: those rows fall outside every timeline window the interface
-- offers, so the screen looks broken for a reason nobody would guess.
--
...
VALUES
  ('seed-audit-1', 'seed-classifier', 'seed-task-done-1', 'file_read',
   ...
   datetime('now', '-4 minutes'), 34, 0, 1, NULL, '{"cpu_ms":12,"peak_rss_kb":8100}',
  ...
  ('seed-audit-2', 'seed-classifier', 'seed-task-done-1', 'web_search',
   ...
   datetime('now', '-11 minutes'), 812, 0, 1, NULL, '{"cpu_ms":56,"peak_rss_kb":21400}',
  ...
  ('seed-audit-3', 'apollia-guide', 'seed-task-done-2', 'file_write',
   ...
   datetime('now', '-26 minutes'), 47, 0, 1, NULL, '{"cpu_ms":9,"peak_rss_kb":7600}',
  ...
  ('seed-audit-4', 'apollia-guide', 'seed-task-error-1', 'bash_executor',
   ...
   datetime('now', '-38 minutes'), 1503, 2, 0, 'ExitNonZero', '{"cpu_ms":140,"peak_rss_kb":33200}',
  ...
  ('seed-audit-5', 'seed-classifier', 'seed-task-run-1', 'file_read',
   ...
   datetime('now', '-52 minutes'), 28, 0, 1, NULL, '{"cpu_ms":11,"peak_rss_kb":8000}',
  ...
  ('seed-audit-6', 'apollia-guide', 'seed-task-done-2', 'web_search',
   ...
   datetime('now', '-3 hours'), 1140, 0, 1, NULL, '{"cpu_ms":74,"peak_rss_kb":22800}',
